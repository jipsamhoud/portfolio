...
- **2026-09-22 — PORTRETTENSTRIP (verzoek Jip): de fotogalerij onderaan Beyond Business is nu een horizontale slider met 10 portretten.** De statische 3-tegel-grid is vervangen door een scroll-snap-filmstrip (zelfde tegelformaat 4/5, zelfde grayscale-filter): swipe op mobiel (tegels 76vw), pijltjesknoppen ←/→ plus teller "01 / 10" in mono-stijl rechts in de kopregel naast "NL2025 · 2025". Acht nieuwe portretten door Jip aangeleverd: `assets/photo-4.webp` t/m `photo-11.webp` (geoptimaliseerd, 1000px hoog, 20-90KB); photo-11 (vrouw, profiel) is op verzoek van Jip opgelicht (gamma 0.55, gemiddelde luminantie 22→52, vergelijkbaar met de donkerste bestaande portretten). Volgorde bewust afwisselend man/vrouw: 1,2,3,5,11,4,8,6,9,7,10 (vrouwen op posities 1,3,5,7,9). Op mobiel tonen tegels op 44vw (twee per schermbreedte) zodat de strip compact aanvoelt. In de kopregel is de boektitel nu een link naar bol.com ("Samen maken we Nederland"). Techniek: CSS scroll-snap + klein los script onderaan de body (event-delegatie, raakt de render-runtime niet); CSS in het helmet-blok (`[data-photo-strip]`, scrollbar verborgen, hover op knoppen). **Designtaak: neem de nieuwe markup (strip + knoppen/teller-rij), de CSS, het script en de 7 nieuwe assets op in de Design-bron** — anders draait de volgende export dit terug naar 3 foto's.
- **2026-09-22 — NIEUW FAVICON: "JS"-monogram (keuze Jip).** `assets/favicon.png` vervangen (was: losse "J" in systeemfont, 64×64) door een JS-monogram in Archivo (weight 800, strakke tracking) wit op merkblauw #3554f4, 96×96. Nieuw bestand `assets/apple-touch-icon.png` (180×180) voor iOS. In de head: `sizes="96x96"` + cache-buster `?v=2` op het icon-link en een nieuwe apple-touch-icon-regel. **Designtaak: neem beide bestanden en de twee head-regels op in de Design-bron**, anders draait de volgende export ze terug.
- **2026-09-22 — kleine designtaak (uit Bing URL Inspection):** de meta description is 175 tekens; Bing vlagt alles boven 160 ("Meta Description too long or too short"). Suggestie: kort de description tot ≤160 tekens, bijv. door "from the world's first VR cinema to Let's Copilot" te vervangen door een kortere staart — zelfde wijziging dan ook in og:description.
